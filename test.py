...
class ClfDataset(Sequence):
    ...
    def __getitem__(self, item):
        ...
        with np.load(os.path.join('train_val', '%s.npz' % name)) as npz:
...
dataset = ClfDataset(crop_size=36,move=5)
dataset_val = ClfDataset_val(crop_size=36,move=5)
test_dataset = ClfDataset_test(crop_size=36,move=5)
...
PARAMS = {
    ...
    'dropout_rate': 0.1  # whether to use dropout, and how much to use
}
...
model = get_model('weights.19.h5py')
...
submission['Score']=b[0]
save=pd.DataFrame(data = submission)
save.to_csv('submmission.csv')
